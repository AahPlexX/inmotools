...
export function prepareCodecSource(file: Blob, format: string, seconds: number, createWorker: () => Worker, settings?: PhotoRawSettings): Promise<PhotoImportRaster> {
  const task = queue.then(async () => {
    ...
    try {
      return await new Promise<PhotoImportRaster>((resolve, reject) => {
        ...
        worker.onmessage = (event: MessageEvent<{ blob?: Blob; notice?: string; error?: string; rawSource?: PhotoRawSource }>) => finish(() => {
        ...
        try { worker.postMessage(settings ? { buffer, settings } : buffer, [buffer]); }
