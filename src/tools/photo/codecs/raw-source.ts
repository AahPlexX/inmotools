...
export function prepareRawSource(file: Blob, settings?: PhotoRawSettings, onPreview?: (blob: Blob) => void): Promise<PhotoImportRaster> {
  ...
  return prepareCodecSource(file, 'RAW', 30, () => new Worker(new URL('./raw.worker.ts', import.meta.url), { type: 'module' }), options, onPreview);
}
