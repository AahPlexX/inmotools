...
export default function SubtitleWorkspace() {
  ...
  return <><div className="workspace-header"><div><h2>Two-anchor correction</h2><p>All cue timestamps use the same calculated slope and offset.</p></div></div><div className="workspace-body">
    <div className="field"><label htmlFor="subtitle-text">Subtitle file contents</label><textarea id="subtitle-text" value={text} onChange={(event)=>setText(event.target.value)} /><small>{parsed ? `${parsed.cues.length} cues detected · ${parsed.format.toUpperCase()}` : 'Check subtitle syntax.'}</small></div>
