...
**Current verified count:** 16/60
**Current executable evidence:** Rows 20, 22, 23 and 24 are implemented and awaiting the exact-source browser gate at `c6550af2a0b32f970983e2aed2c70fed68817346`. On that revision the repository unit suite and production/PWA build are green; workflow run `36204645295` is executing the browser matrix. Row 19 remains verified from `69bec32e29dbed806c4439b9a80314c918e4dda2`. The verified numerator remains 16/60 until browser evidence closes the four interactive rows.
...
| 20 | Temporal Visibility Spans | implemented | `timeline-engine.ts`, `TacticalTimingControls.tsx`, `TacticalMatchboardWorkspace.tsx`, timeline/e2e tests | Generalized immutable visibility authoring now covers active-scene layers, players, equipment, annotations and scene objects through timeline targets; sampled presentation applies authored visibility without mutating canonical state. Unit/build evidence is green at `c6550af`; exact desktop/mobile browser verification is pending run `36204645295`. |
...
| 22 | Timeline Markers & Coaching Triggers | implemented | `tactics-types.ts`, `timeline-engine.ts`, `TacticalTimelinePanel.tsx`, timeline/e2e tests | Typed pass, press, line-break, switch, shot, transition and coaching-cue markers are validated, authored and displayed. Unit/build evidence is green at `c6550af`; exact desktop/mobile browser verification is pending run `36204645295`. |
| 23 | Coordinated Tactical Action Templates | implemented | `action-engine.ts`, `TacticalCoordinationControls.tsx`, action/e2e tests | Editable named overlap, underlap, third-player, wall-pass, switch, give-and-go, decoy, press and recovery patterns map practical two/three-player roles to canonical timeline motion while preserving a custom authoring path. Unit/build evidence is green at `c6550af`; exact desktop/mobile browser verification is pending run `36204645295`. |
| 24 | Linked Defensive / Midfield / Attacking Units | implemented | `unit-engine.ts`, `TacticalCoordinationControls.tsx`, unit/e2e tests | Linked members now support translation plus explicit line-shift, step, drop, width and depth adjustments with direction-of-play-aware forward/backward semantics and normalized-bound validation. Unit/build evidence is green at `c6550af`; exact desktop/mobile browser verification is pending run `36204645295`. |
