...
const isPalette = (value: unknown): value is FiberCraftDocument['palette'] => {
  if (!Array.isArray(value) || value.length === 0) return false;
  const ids = new Set<string>();
  return value.every((color) => {
    if (!isRecord(color) || typeof color.id !== 'string' || color.id.trim() === ''
      || typeof color.label !== 'string' || typeof color.hex !== 'string' || !COLOR_HEX.test(color.hex)
      || (color.paletteCode !== undefined && typeof color.paletteCode !== 'string')
      || (color.paletteName !== undefined && typeof color.paletteName !== 'string') || ids.has(color.id)) return false;
    ids.add(color.id);
    return true;
  });
};
...
const isPolarChart = (value: unknown, paletteIds: ReadonlySet<string>): value is PolarChart => {
  ...
  const validNodes = value.nodes.every((node) => {
    ...
    const round = Number(node.round); const angleIndex = Number(node.angleIndex); const stitchesInRound = Number(node.stitchesInRound);
    const key = `${round}:${angleIndex}`; if (seen.has(key)) return false; seen.add(key);
...
const isHalfGridPoint = (value: unknown, chart: Pick<CountedThreadChart, 'rows' | 'cols'>): value is CountedThreadPoint => {
  if (!isRecord(value)) return false;
  const row = Number(value.row); const col = Number(value.col);
  return Number.isFinite(row) && Number.isFinite(col) && Number.isInteger(row * 2) && Number.isInteger(col * 2)
...
const isCountedThreadChart = (value: unknown, paletteIds: ReadonlySet<string>): value is CountedThreadChart => {
  ...
  const bounds = { rows, cols }; const cellKeys = new Set<string>();
  if (!value.cells.every((cell) => {
    if (!isRecord(cell)) return false;
    const row = Number(cell.row); const col = Number(cell.col); const key = `${row}:${col}`; if (cellKeys.has(key)) return false; cellKeys.add(key);
    const stitchKind = cell.stitchKind; const colorId = cell.colorId;
    return Number.isInteger(row) && row >= 0 && row < rows && Number.isInteger(col) && col >= 0 && col < cols
      && (stitchKind === null || isCountedStitchKind(stitchKind))
      && (colorId === null || (typeof colorId === 'string' && paletteIds.has(colorId)))
      && ((stitchKind === null && colorId === null) || (stitchKind !== null && typeof colorId === 'string'));
  })) return false;
  const knotIds = new Set<string>();
  if (!value.knots.every((knot) => {
    if (!isRecord(knot) || typeof knot.id !== 'string' || knot.id.trim() === '' || knotIds.has(knot.id) || typeof knot.colorId !== 'string' || !paletteIds.has(knot.colorId) || !isHalfGridPoint(knot.point, bounds)) return false;
    knotIds.add(knot.id); return true;
  })) return false;
...
export const isRestorableCrochetDocument = (value: unknown): value is FiberCraftDocument =>
  isRestorableFiberCraftDocument(value) && value.metadata.discipline === 'crochet';
  ...
  return {
    async load() {
      ...
      return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readonly'); const request = tx.objectStore(STORE_NAME).get(AUTOSAVE_KEY);
        request.onsuccess = () => { db.close(); resolve(isRestorableFiberCraftDocument(request.result) ? request.result : null); };
        request.onerror = () => { db.close(); reject(request.error ?? new Error('Could not read the local Fiber Craft draft.')); };
      });
    },
    async save(document) {
      const db = await openDb();
      return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite'); tx.objectStore(STORE_NAME).put(document, AUTOSAVE_KEY);
        tx.oncomplete = () => { db.close(); resolve(); };
        tx.onerror = () => { db.close(); reject(tx.error ?? new Error('Could not save the local Fiber Craft draft.')); };
      });
    },
    async clear() {
      const db = await openDb();
      return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite'); tx.objectStore(STORE_NAME).delete(AUTOSAVE_KEY);
        tx.oncomplete = () => { db.close(); resolve(); };
        tx.onerror = () => { db.close(); reject(tx.error ?? new Error('Could not clear the local Fiber Craft draft.')); };
      });
    },
  };
};
