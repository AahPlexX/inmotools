...
export default function ShaderWorkspace() {
  ...
  useEffect(() => {
    ...
    const link = (text: string) => {
      ...
      const fragment = compile(gl.FRAGMENT_SHADER, text);
    ...
    const candidate = link(compiledSource);
    const linked = candidate ?? link(lastLinkedSource);
    setDiagnostics(found);
    if (!linked) {
      setStatus('Shader compilation failed and the previous shader could not be restored.');
      return undefined;
    }
