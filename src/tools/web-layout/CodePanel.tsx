...
export function CodePanel({ project, onChange }: { project: LayoutProject; onChange: (code: CodeProject) => boolean }) {
  ...
  return <section className="wl-code-panel" aria-label="Code workstation">
    ...
    <div className="button-row"><button type="button" onClick={() => { try { if (!onChange(draft)) throw new Error('Code could not be applied. Check the project status above.'); setStatus('Code applied to project. Undo restores the previous project.'); } catch (e) { setStatus(String(e)); } }}>Apply code</button><button type="button" disabled={busy} onClick={() => void format()}>Format draft</button><button type="button" onClick={() => downloadText(draft[language], `web-layout.${language}`)}>Download source</button><button type="button" onClick={() => void zip()}>Download code ZIP</button></div>
