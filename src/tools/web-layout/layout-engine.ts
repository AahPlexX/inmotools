...
.layout{display:${p.layout};gap:var(--space);grid-template-columns:repeat(auto-fit,minmax(min(100%,max(16rem,calc((100% - ${p.columns - 1} * var(--space)) / ${p.columns}))),1fr));flex-direction:${p.direction};flex-wrap:${options.wrap};align-items:${p.alignment};justify-content:${p.distribution}}
...
${p.layout === 'grid' && p.gridAreas.length ? `@container(min-width:calc(${p.columns * 16}rem + ${(p.columns - 1) * p.gap}px)){.layout{grid-template-columns:repeat(${p.columns},minmax(0,1fr));grid-template-areas:${areas}}${placements}}` : ''}
