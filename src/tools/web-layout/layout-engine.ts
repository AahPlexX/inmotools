...
export const INITIAL_PROJECT: LayoutProject = {
  ...
  columns: 3, gap: 24, padding: 32, radius: 16, maxWidth: 1120, breakpoint: 640,
...
export function parseProject(input: string): LayoutProject {
  ...
  const result: LayoutProject = {
    ...
    layout: choice(p.layout, ['grid', 'flex'], 'layout'), columns: number(p.columns, 'Columns', 1, 12),
...
.layout{display:${p.layout};gap:var(--space);grid-template-columns:repeat(${p.columns},minmax(0,1fr));flex-direction:${p.direction};flex-wrap:wrap;align-items:${p.alignment};justify-content:${p.distribution}}
...
@media(max-width:${p.breakpoint}px){.layout{grid-template-columns:minmax(0,1fr);flex-direction:column}.block{flex-basis:auto;width:100%}}
