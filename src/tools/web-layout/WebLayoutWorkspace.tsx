...
function Preview({ project, width, orientation }: { project: LayoutProject; width: number; orientation: 'portrait' | 'landscape' }) {
  ...
  const frameHeight = orientation === 'portrait' ? 680 : 420;
  return <section className="wl-preview" aria-label={`${width} pixel ${orientation} preview`}>
    <div className="wl-preview-label"><strong>{width} px · {orientation}</strong><span>{Math.round(scale * 100)}% view scale</span></div>
    <div ref={host} className="wl-frame-host" style={{ height: Math.max(240, frameHeight * scale) }}>
      <iframe title={`Layout at ${width} pixels, ${orientation}`} sandbox="" referrerPolicy="no-referrer" srcDoc={source}
        style={{ width, height: Math.max(frameHeight, 240 / scale), transform: `scale(${scale})` }} />
    </div>
  </section>;
}
