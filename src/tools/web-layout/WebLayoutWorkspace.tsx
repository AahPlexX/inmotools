...
function Preview({ project, width }: { project: LayoutProject; width: number }) {
    ...
    <div className="wl-body">
      <section className="wl-controls" aria-label={`${tab} controls`}>
        {tab === 'Build' && <>
          ...
          {project.layout === 'grid' ? numeric('Desktop columns', 'columns', 1, 12) : <label className="wl-field">Direction<select value={project.direction} onChange={event => patch({ direction: event.target.value as LayoutProject['direction'] })}><option value="row">Row</option><option value="column">Column</option></select></label>}
      ...
      <div className="wl-preview-list">{widths.map(width => <Preview key={width} project={project} width={width} />)}</div>
    </div>
