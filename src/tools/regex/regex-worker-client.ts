...
export const executeRegexWithWatchdog = (
  ...
  return new Promise((resolve) => {
    ...
    const timer = window.setTimeout(() => {
      worker.terminate();
      resolve({
        engine: flavor === 'pcre2' ? 'PCRE2 10.47.5 · WebAssembly' : flavor === 'oniguruma' ? 'Oniguruma · WebAssembly (vscode-oniguruma 2.0.1)' : 'ECMAScript · browser RegExp',
    ...
    worker.onmessage = (event: MessageEvent<{ requestId: number; result: RegexRunResult }>) => {
