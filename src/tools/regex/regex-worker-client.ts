...
export type RegexExecutionFlavor = 'ecmascript' | 'pcre2' | 'oniguruma';
let requestId = 0;
export const executeRegexWithWatchdog = (flavor: RegexExecutionFlavor, pattern: string, flags: string, subject: string, timeoutMs = 500): Promise<RegexRunResult> => new Promise((resolve) => {
  const id = ++requestId;
