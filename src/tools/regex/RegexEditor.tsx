...
const RegexEditor = ({ label, value, onChange, compact = false }: Props) => {
  ...
  useEffect(() => {
    ...
    const view = new EditorView({ state, parent: host }); view.scrollDOM.tabIndex = 0; view.scrollDOM.setAttribute('aria-label', `${label} scroll region`); viewRef.current = view; return () => { view.destroy(); viewRef.current = null; };
  }, [compact, label]);
