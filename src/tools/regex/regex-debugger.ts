...
export interface RegexDebugTrace { readonly nativeEngineTrace:false; readonly note:string; readonly steps:readonly RegexDebugStep[]; }
...
export const buildDebugTrace=(root:RegexExplanationNode):RegexDebugTrace=>({nativeEngineTrace:false,note:'Structural source traversal, not native engine backtracking steps.',steps:flatten(root).map((step,index)=>({...step,index}))});
