  ...
  return <div className="regex-matrix" data-testid="regex-matrix-workspace">
    ...
    {mode === 'studio' ? <>
      ...
      <main className="regex-studio-grid">
        ...
        <aside className="regex-diagnostic-pane" aria-label="Regex diagnostics"><section><div className="regex-pane-heading"><h2>Explain</h2><span>{explanation.children.length} tokens</span></div><ol className="regex-ast-list">{explanation.children.slice(0,18).map((node) => <li key={node.id}><code>{node.source || '∅'}</code><span>{node.label}</span><small>{node.start}–{node.end}</small></li>)}</ol></section>
