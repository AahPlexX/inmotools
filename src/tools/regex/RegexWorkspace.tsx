...
import RegexPowerWorkbench from './RegexPowerWorkbench';
import RegexPracticeLab from './RegexPracticeLab';
import RegexCustomTracks from './RegexCustomTracks';
import RegexReplacementPreview from './RegexReplacementPreview';
...
import { analyzeCompatibility, REGEX_FLAVOR_CATALOG } from './regex-compat';
...
const initialResult: RegexRunResult = { engine:'ECMAScript · browser RegExp', capability:'execution', matches:[], durationMs:0, startupMs:0, executionMs:0, offsetUnit:'utf16-code-unit', error:null };
...
const RegexWorkspace = () => {
  ...
  const sharedTrack = useMemo(parseSharedTrack, []);
  const [mode,setMode] = useState<RegexMode>(shared?.mode ?? (sharedTrack ? 'academy' : 'studio'));
  ...
  const [academyPane,setAcademyPane] = useState<'lessons'|'practice'|'custom'>(sharedTrack ? 'custom' : 'lessons');
  ...
  const exportMatches = async () => {
    ...
    const payload={ schemaVersion:1, flavor, engine:exported.engine, pattern, flags, subjectLength:subject.length, offsetUnit:exported.offsetUnit, complete, returnedMatches:exported.matches.length, totalMatches:exported.totalMatches ?? (complete ? exported.matches.length : null), limit:exported.matchLimit ?? (flavor === 'ecmascript' ? FULL_EXPORT_MATCH_LIMIT : null), error:exported.error, matches:exported.matches };
    downloadText(JSON.stringify(payload,null,2),'regex-matrix-matches.json','application/json;charset=utf-8');
    setStatus(complete ? `Exported all ${exported.matches.length.toLocaleString()} matches reported by the execution engine.` : `Exported ${exported.matches.length.toLocaleString()} matches with explicit incomplete metadata${exported.matchLimit ? `; the retained-record bound is ${exported.matchLimit.toLocaleString()}` : ''}.`);
  };
  ...
  return <div className={`regex-matrix regex-mobile-${mobileView}`} data-testid="regex-matrix-workspace">
    <header className="regex-matrix-bar">
      ...
      <label>Engine flavor<select id="regex-engine-flavor" aria-label="Engine flavor" value={flavor} onChange={(event) => setFlavor(event.target.value as RegexFlavor)}>{REGEX_FLAVOR_CATALOG.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
    ...
    {mode === 'studio' ? <>
      ...
      <main className="regex-studio-grid">
        <section className="regex-editor-pane" data-mobile-panel="editor" aria-label="Test and replacement workspace"><div className="regex-pane-heading"><h2>Test subject</h2><span>{subject.length} chars</span></div><RegexEditor label="Test subject" value={subject} onChange={setSubject} />
          <div className="regex-substitute"><label>Replacement<input aria-label="Replacement" value={replacement} onChange={(event) => setReplacement(event.target.value)} /></label><RegexReplacementPreview flavor={flavor} pattern={pattern} flags={flags} subject={subject} replacement={replacement} /></div>
        ...
        <section className="regex-match-pane" data-mobile-panel="matches" aria-label="Match inspector">
          ...
          <p className="regex-duration" data-testid="runtime-timing">Total {result.durationMs.toFixed(2)} ms · Startup {(result.startupMs ?? 0).toFixed(2)} ms · Execution {(result.executionMs ?? result.durationMs).toFixed(2)} ms · Offsets use {result.offsetUnit === 'utf16-code-unit' ? 'UTF-16 code units' : 'engine coordinates'}</p>
        </section>
      ...
      <RegexPowerWorkbench flavor={flavor} pattern={pattern} flags={flags} subject={subject} result={result} explanation={explanation} onPatternChange={setPattern} onFlavorChange={setFlavor} onSelectSource={(start,end,label) => { setRailroadSelection((current) => ({ from:start,to:end,revision:(current?.revision ?? 0)+1,label })); setStatus(`Selected ${label} at ${start}–${end}.`); }} />
      ...
      <footer className="regex-exportbar"><button type="button" onClick={() => exportAssertions('json')}>Export test JSON</button><button type="button" onClick={() => exportAssertions('yaml')}>Export test YAML</button><button type="button" onClick={exportCode}>Export code</button><button type="button" onClick={exportDiagram}>Export diagram SVG</button><button type="button" onClick={saveSession}>Save session</button><details className="regex-sessions"><summary>Saved sessions ({savedSessions.length})</summary>{savedSessions.length ? <div className="regex-session-popover"><div>{visibleSessions.map((snapshot) => <div key={snapshot.id} style={{ display:'flex', flexWrap:'wrap', gap:8, alignItems:'center' }}><button type="button" aria-label={`Load saved session ${snapshot.pattern}`} onClick={() => loadSession(snapshot)}><code>{snapshot.pattern || '∅'}</code><span>{new Date(snapshot.savedAt).toLocaleString()}</span></button><button type="button" aria-label={`Delete saved session ${snapshot.pattern}`} onClick={() => deleteSession(snapshot.id)}>Delete</button></div>)}</div><Pager page={safeSessionPage} pages={sessionPageCount} onChange={setSessionPage} label="Saved session pages" /></div> : <p>No saved sessions yet.</p>}</details><details className="regex-shortcuts"><summary>Shortcuts</summary><p><kbd>Ctrl/Cmd</kbd>+<kbd>Enter</kbd> run assertions / check lesson · <kbd>Ctrl</kbd>+<kbd>M</kbd> switch mode (use Control on macOS; Command+M is browser-reserved) · <kbd>Ctrl/Cmd</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd>/<kbd>E</kbd>/<kbd>R</kbd> engine / explain / safety</p></details><button type="button" onClick={() => void share()}>Copy share URL</button><span role="status">{status}</span></footer>
