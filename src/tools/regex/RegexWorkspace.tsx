...
const RegexWorkspace = () => {
  ...
  const [academyPane,setAcademyPane] = useState<'lessons'|'practice'>('lessons');
  ...
  return <div className={`regex-matrix regex-mobile-${mobileView}`} data-testid="regex-matrix-workspace">
    ...
    {mode === 'studio' ? <>
      ...
      <RegexPowerWorkbench flavor={flavor} pattern={pattern} flags={flags} subject={subject} result={result} explanation={explanation} onPatternChange={setPattern} onSelectSource={(start,end,label) => { setRailroadSelection((current) => ({ from:start,to:end,revision:(current?.revision ?? 0)+1,label })); setStatus(`Selected ${label} at ${start}–${end}.`); }} />
