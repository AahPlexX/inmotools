...
test('RegexMatrix Portability applies only exact rewrites and blocks unsafe target migrations', async ({ page }) => {
  await page.goto('./#/regex-matrix');
  await expect(page.getByTestId('regex-matrix-workspace')).toBeVisible();
  ...
  await expect(page.getByTestId('portability-preview')).toContainText('(?P<word>\\w+)\\s+(?P=word)');
  await page.getByRole('button', { name: 'Apply safe rewrite' }).click();
  await expect(page.getByRole('textbox', { name: 'Pattern', exact: true })).toHaveText('(?P<word>\\w+)\\s+(?P=word)');
  await expect(page.getByLabel('Engine flavor')).toHaveValue('python');
  ...
  await expect(page.getByRole('button', { name: 'Apply safe rewrite' })).toBeDisabled();
});
