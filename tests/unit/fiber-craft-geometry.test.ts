...
describe('grid geometry engine', () => {
  ...
  it('sets exactly one cell without mutating the others', () => {
    ...
    const target = updated.cells.find((cell) => cell.row === 0 && cell.col === 1);
    expect(target).toMatchObject({ colorId: 'red', symbolId: 'sc-dc' });
    expect(updated.cells.filter((cell) => cell.colorId !== null)).toHaveLength(1);
  });

  it('rejects a cell outside the grid bounds', () => {
    const chart = createEmptyGridChart(2, 2);
    expect(() => setGridCell(chart, 5, 0, 'red', 'sc-dc')).toThrow();
  });
  ...
  it('mirrors horizontally and vertically without changing cell count', () => {
    ...
    const mirroredH = mirrorGridHorizontal(chart);
    expect(mirroredH.cells.find((cell) => cell.row === 0 && cell.col === 2)?.colorId).toBe('a');
    const mirroredV = mirrorGridVertical(chart);
    expect(mirroredV.cells.find((cell) => cell.row === 1 && cell.col === 0)?.colorId).toBe('a');
  });

  it('rotates a rectangular grid 90 degrees clockwise and swaps dimensions', () => {
    const chart = setGridCell(createEmptyGridChart(2, 3), 0, 0, 'a', 'chain');
    const rotated = rotateGrid90(chart);
  ...
  it('resizing preserves existing in-bounds cells and fills new cells empty', () => {
    const chart = setGridCell(createEmptyGridChart(2, 2), 1, 1, 'a', 'chain');
    const resized = resizeGridChart(chart, 3, 3);
  ...
  it('flags rows whose stitch count does not match the expected count', () => {
    ...
    const findings = findStitchGrowthMismatches(chart, () => 3);
...
describe('polar geometry engine', () => {
  ...
  it('rejects a round with zero stitches', () => {
    expect(() => createEmptyPolarChart([6, 0])).toThrow();
  });
  ...
  it('converts a polar node to a cartesian position centered at the origin', () => {
    const chart = createEmptyPolarChart([4]);
    const node = chart.nodes[0];
    const point = polarNodeToCartesian(node, 10);
...
describe('gauge and physical-dimension math', () => {
  ...
  it('computes stitch density per unit length', () => {
  ...
  it('derives a non-square grid aspect ratio from row/stitch gauge', () => {
    expect(gaugeAspectRatio(gauge)).toBeCloseTo(28 / 20, 5);
  });

  it('rejects a gauge with zero stitch count when computing aspect ratio', () => {
...
describe('crochet symbol library (Craft Yarn Council terminology)', () => {
  ...
  it('throws for an unknown symbol id', () => {
    expect(() => getCrochetSymbol('not-a-real-symbol')).toThrow();
  });
...
describe('knitting symbol library', () => {
  ...
  it('rejects a cable narrower than one stitch per side', () => {
    expect(() => createCableCrossSymbol(0, 'right')).toThrow();
  });
});
