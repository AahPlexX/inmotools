...
describe('GeoJSON simplifier',()=>{
 ...
 it('removes bbox members that would become stale when coordinates are rounded',()=>{const source={type:'FeatureCollection',bbox:[1.04,2.04,1.04,2.04],features:[{type:'Feature',bbox:[1.04,2.04,1.04,2.04],properties:{id:1},geometry:{type:'Point',bbox:[1.04,2.04,1.04,2.04],coordinates:[1.04,2.04]}}]};const rounded=roundGeoCoordinates(source,0);expect(rounded.features[0].geometry.coordinates).toEqual([1,2]);expect('bbox' in rounded).toBe(false);expect('bbox' in rounded.features[0]).toBe(false);expect('bbox' in rounded.features[0].geometry).toBe(false);});
