...
describe('CAD exact line-to-curve tangency', () => {
  it('solves a free line tangent to an axis-aligned ellipse in normalized ellipse space', () => {
    const sketch: CadSketch = {
      id: 'ellipse-tangent', label: 'Ellipse tangent', plane: { kind: 'origin', plane: 'XY' },
      entities: [
    ...
    expect(distanceFromOriginToLine(a.x / 10, a.y / 5, b.x / 10, b.y / 5)).toBeCloseTo(1, 6);
  });
  ...
  it('solves tangency to a rotated ellipse using its local major/minor frame', () => {
    ...
    const vx = -uy;
    const vy = ux;
    const localToWorld = (major: number, minor: number) => ({ x: major * ux + minor * vx, y: major * uy + minor * vy });
    const majorPoint = localToWorld(10, 0);
    const a0 = localToWorld(11, -5);
    const b0 = localToWorld(11, 5);
    const sketch: CadSketch = {
      id: 'rotated-ellipse-tangent', label: 'Rotated ellipse tangent', plane: { kind: 'origin', plane: 'XY' },
      entities: [
        { id: 'center', type: 'point', x: 0, y: 0, construction: false },
    ...
    const a = point(result, 'a');
    const b = point(result, 'b');
    const toNormalized = (x: number, y: number) => ({ x: (x * ux + y * uy) / 10, y: (x * vx + y * vy) / 5 });
  ...
  it('accepts a tangent contact that lies inside the directed circular-arc span', () => {
    const sketch: CadSketch = {
      id: 'arc-valid-tangent', label: 'Arc valid tangent', plane: { kind: 'origin', plane: 'XY' },
      entities: [
        { id: 'center', type: 'point', x: 0, y: 0, construction: false },
    ...
    expect(solveSketch(sketch).converged).toBe(true);
  });

  it('accepts a tangent contact that lies inside the directed elliptical-arc span', () => {
    const sketch: CadSketch = {
      id: 'earc-valid-tangent', label: 'Elliptical arc valid tangent', plane: { kind: 'origin', plane: 'XY' },
      entities: [
        { id: 'center', type: 'point', x: 0, y: 0, construction: false },
        { id: 'major', type: 'point', x: 10, y: 0, construction: false },
  ...
  it('rejects tangency whose contact lies on the excluded half of a circular arc', () => {
    const sketch: CadSketch = {
      id: 'arc-tangent-span', label: 'Arc tangent span', plane: { kind: 'origin', plane: 'XY' },
      entities: [
        { id: 'center', type: 'point', x: 0, y: 0, construction: false },
        { id: 'start', type: 'point', x: 5, y: 0, construction: false },
        { id: 'end', type: 'point', x: -5, y: 0, construction: false },
  ...
  it('rejects tangency whose contact lies on the excluded half of an elliptical arc', () => {
    const sketch: CadSketch = {
      id: 'elliptical-arc-tangent-span', label: 'Elliptical arc tangent span', plane: { kind: 'origin', plane: 'XY' },
      entities: [
        { id: 'center', type: 'point', x: 0, y: 0, construction: false },
        { id: 'major', type: 'point', x: 10, y: 0, construction: false },
        { id: 'start', type: 'point', x: 10, y: 0, construction: false },
