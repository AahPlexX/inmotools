...
**Last tracked implementation commit:** `13884f5ffd31a358574a0b8ebe4c49280571c407`
...
- Dimensional RED contracts were added at `f628e4ac2fd79b0051419f221f3465cc8f289125` for horizontal/vertical point distance, explicit line length, circle diameter, and line angle in canonical radians. Aligned run `34643263671` failed exactly those four contracts while 664 tests passed, including all prior CAD sketch suites and the freshness guard.
- Production support for horizontal/vertical distance, explicit line length, circle diameter, and line angle is committed through `469f95f2e42071068b7caa29e8c5ccc8a56e81cf`. After ledger alignment, run `34643533639` passed all 668 unit tests and the production build. Its focused browser stage failed only the concurrently developed Vector Studio route `svg-sprite-compiler` for `scrollable-region-focusable` on `.vector-canvas-scroll` and `.vector-inspector`; no CAD browser route exists yet, so this unrelated failure remains owned by the parallel vector work and final CAD browser/release proof remains G11.
- Formula/reference-dimension RED contracts were added at `143dfa230912d34d3d2a66bef2d3733bc84a085f` for named-parameter/formula binding into canonical driving dimensions, dimensional mismatch rejection, and non-controlling reference measurements. The aligned RED run failed because `sketch-dimensions` did not yet exist while prior CAD suites stayed green.
- Production support for formula-bound driving dimensions and pure reference measurements is committed through `13884f5ffd31a358574a0b8ebe4c49280571c407`. Driving expressions retain serializable source text, resolve through the existing dimensional parameter engine, reject length/angle mismatches, and write canonical values before solve; reference measurements report distance, directional distance, line length, radius, diameter, and angle without mutating or constraining sketch geometry. GREEN validation is pending at this checkpoint.
- The solver now models point/line/circle entities plus fixed-point, fixed-entity, horizontal, vertical, point-distance, horizontal-distance, vertical-distance, line length, radius, diameter, angle, coincident, perpendicular, parallel, tangent, concentric, equal-length, equal-radius, midpoint, point-on-line, point-on-circle, symmetric-points, DOF rank analysis, disabled constraints, soft drag seeding, and explicit conflict reporting. This is not yet the full 17–32 constraint capability set.
