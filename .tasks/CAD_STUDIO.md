...
**Last tracked implementation commit:** `fb5f59fea1344d3e159074f00ca615c48f151c70`
...
- The exact-kernel worker protocol boundary is implemented at `fb5f59fea1344d3e159074f00ca615c48f151c70`. It defines serializable revisioned requests/results, opaque worker-only exact shapes, explicit kernel error codes, preview/final quality, structured operations, zero-copy transferable tessellation/export buffers, current-revision checks, and the internal exact-kernel interface without importing unavailable WASM dependencies. Run `34716588970` passed all four focused kernel-contract tests and **779 tests overall**; only the expected stale progress SHA remained before this tracker-only alignment.
...
- G4 remains open on the numerical-library/dependency policy. G5 protocol work has started, but G5 remains open until verified dependencies are installed and the real OCCT adapter/worker, revision restart/cancellation, analytic fixtures, and exact round-trips pass.
