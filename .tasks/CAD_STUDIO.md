...
**Last tracked implementation commit:** `d100f7a84b9e380537a7f0a5c51cbc1e76e4c8bf`
...
- Finite point-on-curve RED is committed at `db5071c47d8dc923731f2307253dcb80df77e179`. Run `34705473720` proved the intended missing behavior: three finite/full-curve cases failed while an excluded-span fixed-point conflict remained conservatively non-convergent. Uniform point-on-curve typing is committed at `f0709ad64349f9c0cecd8e0a6fc6c89fef832457`; exact solver behavior is committed at `24cbce43b6b4e57eeb77a2804ba16c79462b2f05`. The constraint supports circles, finite directed circular arcs, rotated ellipses, and finite directed elliptical arcs. Run `34705680912` passed all four new point-on-curve tests and 747 tests overall; tracker-aligned run `34705743969` then passed the full unit suite and production build.
- Safe inference-acceptance RED is committed at `99b541d3dadfe3b0ccbb34c2bb2004fe8f6a41bd`; run `34705807731` failed all four new acceptance cases for the intended missing API while the prior 747 tests remained green. Transactional acceptance is implemented at `b40cd04091bd33a6c0dfd27b86c2ec93baef9780`: only inference-emitted relationship types are accepted, semantic duplicates are rejected independently of IDs, source sketches remain untouched, and the candidate sketch is solved before acceptance so stale/unsatisfiable proposals never commit. Run `34705865267` passed all four acceptance tests and 751 tests overall; tracker-aligned run `34705958397` passed the full unit suite and production build.
- Generalized tangency RED is committed at `3bc7180bb4c47ca52cc739110d0856033dcc9cc8`; the RED contract was deliberately hardened at `2bfd1b1415809d9a2829d75a982c2f88fa832bcf` after the first version revealed that excluded-span non-convergence alone could not prove implementation. Hardened run `34706075375` required valid full-ellipse, rotated-ellipse, circular-arc, and elliptical-arc tangencies to converge while retaining excluded-span rejection. The new `tangent-curve` type is committed at `ff7b040c2004ceb18adef20b8314178fca843695`; exact solver behavior is committed at `d100f7a84b9e380537a7f0a5c51cbc1e76e4c8bf`. Ellipse tangency is analytic in normalized unit-circle coordinates, rotated ellipse frames are respected, and finite circular/elliptical arcs add directed contact-span residuals so tangency to the excluded portion cannot pass. Run `34706304775` passed all six generalized tangency tests and **759 tests overall**; only the expected stale progress SHA remained before this tracker-only alignment.
...
- G4 remains open: true spline curve evaluation/constraints and the numerical-library/dependency policy remain outstanding. Ellipse minor radius is still serialized scalar geometry rather than a solver variable, so ellipse radius-family semantics must be designed rather than guessed. Foundations alone do not count as completed user-facing capabilities.
