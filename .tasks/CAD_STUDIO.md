...
- Concrete browser probe `9b93a07216c9a82c3958a08f0dce414e6ae47d90` uses the pinned `wasm-feature-detect@1.9.0` detectors for SIMD, tail calls, and WebAssembly exception handling and short-circuits safely when WebAssembly itself is unavailable.
- Dedicated worker entrypoint `eef8c31520c6c985640e33ef7b2b64afcb12dbe7` composes the concrete browser probe, mandatory capability gate, one lazy OCCT executor per worker lifetime, and the existing revisioned runtime without exposing native handles. Run `34729150260` passed both worker-entry fixtures and all prior coverage for **829 passing tests** before ledger alignment.
- The worker-executor narrowing correction `09c683bea0f45305a2b7a91a39beeff61a9a19f7` removed an over-narrow helper boundary. Aligned run `34729336137` passed **830/830 unit tests and the production TypeScript/Vite build**, proving the root-cause fix.
- Main-thread worker factory `09536b94fbe3d2c5cf2d00262053915f526fa11a` uses Vite's literal `new Worker(new URL('./cad.worker.ts', import.meta.url), { type: 'module' })` form, provides an explicit unsupported-worker error, and returns the existing replaceable worker transport type. Run `34729436992` passed both factory fixtures and all prior functional coverage for **831 passing tests**; only the deliberately stale tracker guard failed before this alignment.
- G5 remains open for explicit Vite worker/WASM bundle emission proof and final worker cleanup/lifetime evidence. Route registration remains intentionally deferred so CAD's heavy worker/WASM graph cannot enter the generic application bundle before the CAD workspace is registered lazily.
