...
**Last tracked implementation commit:** `80c2d1a94c4120cf2a3aef70f8a4b72a56db6c21`
...
- G4 remains open: tangent and point-on-arc constraints involving an arc are deliberately deferred rather than approximated, because both require angular-span/clockwise handling to avoid silently treating a point on an arc's full underlying circle as if it were on the arc itself. Ellipse, elliptical-arc, and spline entities can now be fixed in place but still have no other geometric/dimensional constraint (concentric, tangent, point-on-curve, radius-family) or DOF/conflict coverage of their own. Foundations alone do not count as completed user-facing capabilities.
