...
**Last tracked implementation commit:** `f3f8a45b74e5045e65e7af0b891fa78347e282a3`
...
- G6 remains open. The real remaining blocker for extrude/revolve/sweep/loft is architectural, not mechanical: `occt-adapter.ts` already forwards a generic `profile: CadKernelShape` straight into real `kernel.extrude/revolve/sweep/loft`, and `occt-wasm` already exposes the low-level primitives needed to build one (`makeLineEdge`, `makeArcEdge`, `makeCircleEdge`/`makeCircleArc`, `makeEllipseEdge`/`makeEllipseArc`, `makeBSplineEdge`, `makeWire`, `makeFace`) — but nothing yet turns a solved `CadSketch` (G4) plus a chosen closed profile region (from `sketch-diagnostics.ts`) into 3D-placed edges via the sketch's `SketchPlane` (origin/datum/face), and `CadProject.sketches` is still the untyped placeholder `Record<string, unknown>[]` rather than real `CadSketch[]`. Whoever picks this up next should design the sketch-to-wire/face conversion and the `CadProject.sketches` typing together, since one is not usable without the other; this was deliberately left as a flagged decision rather than freelanced, given other concurrent CAD agents may already be mid-way through it.
