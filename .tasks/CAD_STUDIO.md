...
**Last tracked implementation commit:** `6801c6606a7508c548c773e68263a90edd291e7e`
...
- True interpolating B-spline evaluation is implemented at `6801c6606a7508c548c773e68263a90edd291e7e`. It supports clamped open and periodic closed splines, degree 1–5, chord-length fit parameters, exact fit equations, optional open-spline endpoint derivative constraints, deterministic minimum-norm interpolation for underdetermined systems, and explicit closed-spline tangent-handle rejection. Run `34706667989` passed all five spline-evaluator tests and **764 tests overall**; only the expected stale progress SHA remained before this tracker-only alignment.
...
- G4 remains open: the real spline evaluator must be integrated into spline membership/tangency constraints, and the numerical-library/dependency policy must be satisfied. Ellipse minor radius remains serialized scalar geometry rather than a solver variable, so ellipse radius-family semantics must be designed rather than guessed. Foundations alone do not count as completed user-facing capabilities.
