...
**Last tracked implementation commit:** `df93fbeb4eb7a50468615092db8175c4190efe6e`
...
- True spline contact constraints are implemented at `8b54e05f1d098146226e46a88c1d9010b0e73ccb` after the serializable contact-parameter contract at `d256f8c70d8261357cc804bbea699b8580342594`. Spline-backed point-on-curve and tangent-curve constraints allocate a bounded solver-owned contact parameter, evaluate the spline from current solver geometry, include contact state in Jacobian/DOF analysis, and persist the solved parameter for deterministic replay. Run `34716324971` passed all three focused spline-contact tests and **774 tests overall**; tracker-aligned run `34716384305` passed the full unit suite and production build.
- The exact-kernel worker protocol boundary is implemented at `fb5f59fea1344d3e159074f00ca615c48f151c70`. It defines serializable revisioned requests/results, opaque worker-only exact shapes, explicit kernel error codes, preview/final quality, structured operations, zero-copy transferable tessellation/export buffers, current-revision checks, and the internal exact-kernel interface without importing unavailable WASM dependencies. Run `34716588970` passed all four focused kernel-contract tests and **779 tests overall**; tracker-aligned run `34716646196` passed the full CAD-aware unit suite and production build before browser setup.
- Worker generation/revision coordination is implemented at `df93fbeb4eb7a50468615092db8175c4190efe6e`. `CadKernelSession` issues strictly increasing revisions, can invalidate without replacement work, advances both revision and generation for hard worker restart, rejects old-generation/stale responses, and permits a current response to be consumed only once. Run `34716731465` passed all four focused session tests; the remaining validation failures were the expected stale CAD progress SHA and a concurrent unrelated Crystal export module missing from that synthetic merge.
- Fresh dependency verification still corroborates `occt-wasm@5.0.0`, `manifold-3d@3.5.3`, and `ml-matrix@6.15.0`. The current connected execution environment does not expose a repository shell/package-manager write path, so `package.json`/`pnpm-lock.yaml` remain intentionally untouched rather than hand-edited; dependency installation remains a mechanical G4/G5 blocker.
...
- G4 remains open on the numerical-library/dependency policy. G5 protocol/revision work has started, but G5 remains open until verified dependencies are installed and the real OCCT adapter/worker, browser capability handling, analytic fixtures, and exact round-trips pass.
- `main` contains unrelated concurrent work; CAD remains isolated until G15 reconciliation. Fresh compare after the kernel-contract slice showed CAD branch changes still confined to CAD code/tests/docs/tracker despite branch divergence.
