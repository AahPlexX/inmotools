...
**Last tracked implementation commit:** `eef8c31520c6c985640e33ef7b2b64afcb12dbe7`
...
- Dedicated worker entrypoint `eef8c31520c6c985640e33ef7b2b64afcb12dbe7` composes the concrete browser probe, mandatory capability gate, one lazy OCCT executor per worker lifetime, and the existing revisioned runtime without exposing native handles. Run `34729150260` passed both worker-entry fixtures and all prior coverage for **829 passing tests**; only the deliberately stale tracker guard failed before this alignment.
- G5 remains open for main-thread module-worker creation, route-lazy WASM asset/build verification, worker cleanup/lifetime verification, and a clean aligned production-build checkpoint. Revision/restart, exact adapter, exact import/export state, concrete unsupported-browser detection, and the dedicated worker entrypoint are implemented.
