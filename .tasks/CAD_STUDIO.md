...
**Last tracked implementation commit:** `293d7fe80a640bb2bab616fbdd9d75bcaca1925e`
...
- G6 remains open. Booleans, extrude/revolve/sweep/loft, offset, fillet/chamfer, shell, and mirror are implemented against the real kernel with semantic topology resolution where applicable. Still unimplemented in the evaluator: hole, thicken/draft/split/rib, pattern/helix/thread/text, datum planes/axes as real placement (currently pass-through only), surfaces, defeaturing, and healing. `split` and `pattern` each carry the same open design question — a feature producing more than one output shape doesn't fit the current one-shape-per-feature evaluator model and needs a decision (keep one piece and warn, or extend the model) before implementation, not a guess.
