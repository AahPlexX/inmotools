...
- Pull request: `#29`
- Current material Photo head before this tracker-only commit: `fc89cb5f414ce1a2955188a19252ae19f3b58dba`
- Current phase: **Capability expansion Phase 1 / Task 1.5 — capability-gated RAW adapter**
- Current verification state: material Photo head `fc89cb5f414ce1a2955188a19252ae19f3b58dba` has fresh local evidence for 121 Photo unit/selector tests across 14 files, the production/PWA build, and all 78 focused Photo browser cases across desktop/mobile Chromium with one browser worker. TIFF is connected to acquisition, comparison, preview, export/batch, and durable recovery through one lazy raster path. Eight new browser cases verify original persisted bytes, literal before/export pixels, 16-bit disclosure, Deflate support, unsupported-variant preservation, and stale TIFF rejection. No new parallel review agents were used for this batch.
- Latest inspected PR workflow context: run `34974201415` at tracker head `f82bc7a4000954196dc68b411db7fd5c0eebcbf7` recorded 805 passing repository tests and three failing `vector-engine-path-motion` assertions; those failures are outside Photo Studio scope and predate TIFF. Build/browser validation was skipped. The earlier missing Crystal module is no longer a failure in this inspected run. Post-TIFF remote checks remain unverified; local Photo GREEN is not an integrated-release claim.
- Test-count correction: the earlier recorded 110-case Photo unit/selector total was not the exact Photo namespace count. PR run `34974201415` and the current deterministic filter establish a 93-case baseline before TIFF. This batch adds 24 TIFF cases and four import cases, yielding 121; no tests were removed. Use `pnpm exec vitest run tests/unit/photo tests/unit/e2e-spec-selection.test.mjs --maxWorkers=1` for reproducible scoped counts.
...
- [ ] Phase 1 / Task 1.5 — capability-gated RAW adapter.
...
1. Implement Phase 1 / Task 1.5 with the capability-gated RAW adapter; reverify current official documentation and npm, supported formats, worker API, license, bundle behavior, and exact stable dependency pins immediately before installation without `^` ranges.
...
- Conservative full-wiring checkpoint: **26 of 164 capabilities verified; 138 remain incomplete**, including partially implemented items. This carries forward the previous 25-item audit and promotes only capability 5 (lazy TIFF import) in this batch; it is not a new full-inventory audit.
- Full-wiring IDs: **1, 2, 3, 4, 5, 11, 13, 14, 15, 16, 17, 18, 19, 21, 26, 27, 28, 30, 32, 43, 88, 89, 96, 138, 139, 153** in `docs/superpowers/specs/2026-09-12-photo-studio-capability-expansion.md`.
