...
- Pull request: `#29`
- Current material Photo head before this tracker-only commit: `601d982c0a0d3081e1509b3dd702ff7e74dc58ed`
- Current phase: **Capability expansion Phase 1 / Task 1.5 — capability-gated RAW adapter**
- Current verification state: material Photo head `601d982c0a0d3081e1509b3dd702ff7e74dc58ed` has fresh local evidence for 158 Photo unit/selector tests across 15 files (8.86 seconds), the production/PWA build, and all 88 focused Photo browser cases across desktop/mobile Chromium with one browser worker (4.7 minutes), with no failures or skips. Two added browser cases verify source-gated RAW development, undo/redo, durable recovery, unchanged default-developed before pixels, camera/source inspection, matching preview/single/batch output pixels, failed-item isolation, individual/global RAW resets, coherent typed decimal undo, and 320 CSS px reflow. Existing RAW/TIFF, presets, project, comparison, and editor contracts remain GREEN. The final source build was reused unchanged for the consolidated browser gate; no parallel review agents were used.
- Latest inspected PR workflow context: run `34979321025` at tracker head `a6f6ee35395d482947293f19003f4dae0d2b861a` recorded 853 passing repository tests and three failing `vector-engine-path-motion` assertions; those failures are outside Photo Studio scope and predate TIFF. Build/browser validation was skipped. Post-development remote checks remain unverified; local Photo GREEN is not an integrated-release claim. Latest inspected `origin/main`: `ac31e5b1e40fe4255374319bca64cc2864e494c8`; its updates since the prior checkpoint affect another workstream and shared task records, not Photo source. Main was not merged, rewritten, or pushed by this batch.
- Test-count correction: the earlier recorded 110-case Photo unit/selector total was not the exact Photo namespace count. PR run `34974201415` establishes 93 before TIFF; TIFF adds 28, yielding 121; RAW acquisition adds 20, yielding 141; RAW development adds 16 normalization/history/source/pixel cases and one variant-cache race case, yielding 158. No tests were removed. Use `pnpm exec vitest run tests/unit/photo tests/unit/e2e-spec-selection.test.mjs --exclude '**/.claude/**' --maxWorkers=1` for reproducible scoped counts. The explicit exclusion prevents duplicate tests from unrelated nested checkouts and leaves those files untouched.
...
- [x] RAW acquisition at `5d43c20e811145d2c2b05ba7fc79ac3d62b890f6` now has source-gated development and inspection at `601d982c0a0d3081e1509b3dd702ff7e74dc58ed`: camera/daylight/custom RGB white balance, clip/unclip/blend highlight handling, practical Bayer interpolation choices, read-only camera/source facts, normalized independent recipe history, durable recovery, and shared preview/single/batch rendering. This is partial capability 8, not closure of exposure baseline or embedded previews.
...
- [ ] Phase 1 / Task 1.5 — RAW acquisition implemented at `5d43c20e811145d2c2b05ba7fc79ac3d62b890f6`; supported white-balance/highlight/Bayer demosaic controls and camera/source inspection implemented at `601d982c0a0d3081e1509b3dd702ff7e74dc58ed`. Finish RAW exposure baseline and embedded previews before closing this task.
...
- Conservative full-wiring checkpoint: **27 of 164 capabilities verified; 137 remain incomplete**, including partially implemented items. This carries forward the RAW-acquisition checkpoint; it is not a new full-inventory audit. Capability 8 now includes real RAW white-balance/highlight/Bayer demosaic controls and camera/source inspection, but remains partial because exposure baseline is not implemented. Capability 7 remains open. Ordinary post-raster adjustments are not credited as sensor-level development.
...
- Open: embedded previews (7), RAW exposure baseline needed to finish capability 8, and broader representative legal proprietary RAW fixtures. The selected wrapper has no exposure-shift binding; existing raster exposure remains separate. Keep Task 1.5 open and preserve the full accepted goal rather than silently excluding the unsupported setting.
