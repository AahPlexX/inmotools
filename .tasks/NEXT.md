...
`dedupe.spec.ts`'s remaining 4 failures are **not** this class of fix — they're genuine, unimplemented features already described further down in this file: no export-progress reporting (test expects a `window.__dedupeStatuses` entry matching "Preparing reconciled export"), no encoding selector (`#dedupe-encoding` doesn't exist in the DOM at all, so Windows-1252 CSVs can't be decoded), no review-column visibility toggle for wide datasets, and no transitive-cluster-confidence labeling. These need real implementation work, not a locator or wording fix.
