...
- **Tabular Sheet Workstation (TASK-022)** — PRODUCT CUT P1–P16 DONE. Sole writer on `feature/tabular-sheet-parity` (PR #71 only; CoS squash-merges; agent does not merge or mark ready; do not open a second PR; do not invent extra product scope; do not expand PX). Do not shrink Insert Function to XLOOKUP-only.
  ...
  - Tip SHA: `a92d80d56d081b4e927d2d503617b5e454117eca` (update this line in the same cycle as any later push). Last focused-gate code: `c37fff4c1bdd98827266be6f6fd611901756e75e`. Client loop green on `c37fff4` / `3e87e0a`.
  - Synced `origin/main` `b582c34` (#71 squash) with a clean merge; no conflicts; no other-tool overwrites. Sheets gates at the client-loop tip: focused units 42/42; `pnpm build` pass; desktop-chromium 24 passed; P16 matrix 12/12. Do not claim the full Pages suite green.
  - Known out-of-suite CI reds (do not chase): web-layout-studio axe; svg-sprite-compiler axe; Hardware Packet Inspector; GeoJSON Simplifier; Python re / regex named groups; crystal-lattice-studio mobile.
