...
- **Markdown audit (2026-09-16, continued 2026-09-19)** — source highlighting, discoverable syntax guide, touch formatting/search, and draft/file race guards implemented (2026-09-16). Preview fenced-code language coloring implemented and browser-verified (2026-09-19): `code-highlight-engine.ts` statically highlights ~24 common fenced-code languages via `@lezer/highlight` + this catalog's existing `StreamLanguage`/`@codemirror/legacy-modes` pattern; Mermaid/Graphviz fences are excluded since their `pre` is replaced wholesale by the diagram renderer. A dead, contradictory `ExportAsset` type in `markdown-types.ts` (never imported; shape disagreed with the real one in `export-assets.ts`) was removed. `tsc`, all 1240/1241 repo-wide unit tests (the one unrelated failure is Crystal's `e2e-spec-selection.test.mjs`, confirmed pre-existing via `git stash -u`), production build, and the full 43-test Markdown Playwright suite (desktop Chromium, including 3 new tests for this change) all pass. Dedicated delayed-file-read/save-during-typing browser regression coverage and Markdown Workbench's synchronous main-thread table-formula evaluation (this file's `TASK-014` entry) remain open, per rationale in `docs/markdown-audit-2026-09-19.md`. Scope and remaining checks: `docs/markdown-audit-2026-09-16.md`, `docs/markdown-audit-2026-09-19.md`.
