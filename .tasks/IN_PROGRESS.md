...
- **Photo Studio** — local-first non-destructive photo editor with 30+ functional editing capabilities, professional export/metadata workflow, responsive accessibility, focused validation, and Pages verification.
  ...
  - Current milestone (2026-09-24): capability expansion Phases 3–6 are complete on `claude/photo-tool-completion-ama1g1` (continuation of `feat/photo-studio`); the conservative ledger is 150/164. The remaining scattered capabilities are next, then Phase 7 integration. Phase 1 / Task 1.5 remains an explicit release blocker because the current RAW wrapper does not expose LibRaw pre-demosaic exposure shift. Authoritative boundaries, evidence, remaining IDs, and next-agent instructions: `.tasks/PHOTO_STUDIO.md`.
