...
- **Tactical Matchboard Studio** — 60-feature local-first tactical authoring, animation, spatial-analysis, local-video, presentation, persistence, and export workstream on `feature/tactical-matchboard-studio`.
  ...
  - Current milestone: **Task 3 — unregistered beginner board/workspace slice authored; executable gate active.** T03-01 is the only actionable queue item. Focused tactical units are green **22/22** at `7707b8102699e05841fa8a26f8e42315fa000e87`; production TypeScript/Vite build evidence remains open. Route/catalog registration remains blocked until T03-01 is fully green.
  ...
  - Current evidence limitation: the normal verifier `corepack pnpm build` entrypoint did not reach TypeScript/Vite because its nested bare `pnpm` resolved to 11.24.0 instead of repo-required 12.3.4. Corepack 12.3.4 was activated and the equivalent prepare + `tsc` + Vite sequence was started, but the remote session became unreadable before the outcome could be captured. Do not raise the verified numerator until the production build and later interaction-specific gates are proven.
