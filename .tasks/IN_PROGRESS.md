...
- **Tabular Sheet Workstation (TASK-022)** — PRODUCT CUT P1–P16 DONE. Sole writer on `feature/tabular-sheet-parity` (PR #71 only; CoS squash-merges; agent does not merge or mark ready; do not open a second PR; do not invent extra product scope; do not expand PX). Do not shrink Insert Function to XLOOKUP-only.
  ...
  - Tip SHA: `PLACEHOLDER_TIP_SHA` (update this line in the same cycle as any later push). Last focused-gate code: `c37fff4c1bdd98827266be6f6fd611901756e75e`. Client loop green on `c37fff4` / `3e87e0a`.
