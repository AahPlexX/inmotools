...
- **Tabular Sheet Workstation (TASK-022)** — CoS-locked Excel/Sheets first parity slice P1–P16 on `feature/tabular-sheet-parity` (draft PR only; do not merge; do not open a second PR; do not touch unrelated tools). Do not shrink Insert Function to XLOOKUP-only.
  - Stage 1–3 and FEATURE_MATRIX 1–36 stay `done` on `origin/main`. Current milestone: CoS P1–P16 (paste special; Insert Function full catalog SUM/AVERAGE/IF/VLOOKUP/XLOOKUP/INDEX-MATCH/TEXTJOIN/COUNTIF/SUMIF; AutoSum; named-range manager; Go to/special; hide/unhide+tab color; clear contents vs all; remove duplicates; text to columns; custom number format; CF color scales; list picker; column/line/pie charts; print CSS; keyboard F2/Ctrl-Arrow/Ctrl-;/Ctrl-D fill; device-agnostic client harden). Protect sheet is optional extra PX, not a substitute for P15/P16.
  - Resume: `src/tools/sheets/HANDOFF.md`. Ledger: `src/tools/sheets/FEATURE_MATRIX.md` (1–36 `done` plus CoS P1–P16). Exclusions X1–X10 stay documented (realtime collab, VBA, cloud Power Query, Univer Pro pivots/drawing, HyperFormula, auth/db, FILTER/SORT/UNIQUE formulas, file encryption).
  - Tip SHA: pending this cycle (update this line in the same cycle as any later push).
  - Sheets gates: re-run after this cycle’s code; focused units + `pnpm build` + sheets e2e on desktop-chromium (viewport matrix 320/360/390/412/430 + 740×360); mobile-chromium remaining tests with the generic width loop skipped by design; catalog axe `-g tabular-sheet-workstation`.
