...
- One Phase 1 UX follow-up remains open from the PR review round: touch two-finger pan and pinch-zoom on the schematic canvas (touch currently supports drag-to-place/select/wire but not panning or zooming the viewport by touch; desktop mouse-drag pan and wheel zoom are unaffected). This needs multi-pointer tracking (midpoint delta for pan, distance delta for pinch zoom) alongside the existing single-pointer drag/select/wire handling in `LogicCanvas.tsx`, which is real new-feature scope rather than a bug fix.
