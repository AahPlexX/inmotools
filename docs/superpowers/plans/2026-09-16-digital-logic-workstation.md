...
- Phase 1 UX follow-ups identified during PR review, confirmed real and deferred rather than fixed in Phase 1: touch two-finger pan and pinch-zoom on the schematic canvas (touch currently supports drag-to-place/select/wire but not panning or zooming the viewport by touch; desktop mouse-drag pan and wheel zoom are unaffected); clamping the hover tooltip layer to the canvas viewport bounds (a tooltip near the right or bottom edge can currently be clipped); and centering single-input gate bodies (NOT/BUFFER/TRI-STATE BUFFER) on their pins — the shared minimum-body-height padding used for multi-input gates currently leaves a single-input gate's A/Y pins sitting on the body's top edge instead of centered, in both the canvas renderer and SVG export. Fixing the last one correctly requires re-deriving pin vertical offsets from body-slot centers across `component-library.ts`, `render-engine.ts`, and `export-engine.ts` together so the two renderers cannot drift apart; that is Phase 2 scope rather than a one-file patch.
