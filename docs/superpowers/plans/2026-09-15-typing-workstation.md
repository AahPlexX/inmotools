...
- **B. Focused unit tests — complete.** 66 focused tests across seven convention-discovered `tests/unit/typing-*.test.ts` files cover engine behavior/metrics, correction modes, Enter/newline and Backspace replay, conventional n-gram medians, duration/target sizing, exact ranked corpora, programmer languages, layout-specific finger anchors, key-sound classification, stored/imported-record validation, personal-best identity including Zen/quote family semantics, case-insensitive tag filtering, CSV formula escaping, certificate eligibility, exports, and custom-text normalization.
...
- **Accepted branch revision:** `86b1982ccec67ee34d11cddf187d5a39edfa56a6` (`fix(typing): harden lifecycle and preference boundaries`).
- **Dedicated workflow:** run `35400207813`, job `105778135443`, completed successfully on 2026-09-18. Frozen install/supply-chain policy passed (954 entries), 66/66 focused units across seven files passed, production build passed, Chromium setup passed, and 14/14 desktop/mobile browser checks passed in 45.0s; failure-artifact upload was correctly skipped.
