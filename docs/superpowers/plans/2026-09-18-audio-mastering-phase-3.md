...
- Integration blocker found (2026-09-25): playback currently constructs one AudioBuffer from `currentPcm`, which replays the legacy edit stack; clip references and clip revisions still use original-source offsets, and crop/delete/insert/reverse edits do not update those clip references. Playback must not switch to raw clip sources until this coordinate/content mismatch is resolved and regression-tested.
- Still required before Task 2 can close: resolve the edit-stack/clip-source relationship, render clip arrangements in playback/waveform, define and test gaps and overlaps, expose split/move/duplicate/numeric-and-keyboard nudge controls with accessible alternatives, and verify clip-level undo/redo and marker/region behavior. Do not count ledger 11 or 13 complete until these requirements and the remaining task criteria are met.
