...
- Task 2 partial evidence (2026-09-25): engine/project units pass 29/29; `tsc --noEmit -p tsconfig.app.json` and production Vite build pass after sample-rate-aware clip initialization and frame-aligned revisions. The existing Mastering desktop/mobile Playwright workflow passes 2/2 against the rebuilt app on preview port 4175. Browser coverage includes the current import, transport, range edit, and history workflow; clip arrangement controls and playback rendering remain untested because they are not yet connected.
...
- Source/output mapping foundation (2026-09-25): `mapSourceRangeThroughEdits(source, edits, startSeconds, endSeconds)` in `src/tools/music/mastering-engine.ts` maps an original-source half-open frame range to zero or more final-output ranges ordered by output frame. It replays crop, delete, inserted silence, and reverse operations in order; silence has no source mapping, and content/channel operations preserve timing. Three focused regressions cover crop+delete+silence, reverse splitting/order, and content/channel operations. This metadata helper does not render PCM or model clip-sequence order.
- Integration blocker remains (2026-09-25): playback constructs one AudioBuffer from `currentPcm`, while the project also holds source-backed clip references that can be moved, duplicated, or arranged. The mapping helper does not reconcile later timeline edits with clip-sequence ordering, clip-specific edits, annotations, waveform coordinates, or rendered playback. Keep playback on `currentPcm` until project operation order is defined and rendered arrangement behavior is regression-tested.
- Still required before Task 2 can close: define the project operation/clip-source relationship, render arranged clips in playback and waveform, define and test gaps/overlaps, expose accessible split/move/duplicate/numeric+keyboard nudge controls, and verify clip-level undo/redo and marker/region behavior. Do not count ledger 11 or 13 complete until these requirements and the remaining task criteria are met.
