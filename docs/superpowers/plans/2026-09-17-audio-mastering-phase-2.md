...
- [ ] Add DC measurement/removal, polarity inversion, range reverse, and silence insertion.
- [ ] Add stereo swap, mono fold-down, single-channel extraction, and dual-mono utilities.
- [ ] Prove the focused unit tests red before implementation, then green.

### Task 2 — Reversible edit integration
- [ ] Extend `AudioEdit` replay for the new transforms.
- [ ] Add edit redo alongside existing undo and clear redo on divergent edits.
- [ ] Keep selection/playhead bounded when duration/channel count changes.

### Task 3 — Regions and keyboard transport
- [ ] Add editable marker naming and selection-derived named regions with jump/remove actions.
- [ ] Add non-conflicting global transport shortcuts outside editable/interactive controls: Space play/pause, Left/Right seek, Escape stop, L loop.
- [ ] Expose shortcut help inline rather than relying on hidden behavior.

### Task 4 — UI utility rack
- [ ] Add DC readout/removal, polarity, reverse-selection/whole, silence-duration insertion, and channel operations.
- [ ] Show the working channel count separately from immutable source metadata.
- [ ] Keep the narrow layout stacked and every precision action available without drag.

### Task 5 — Lean acceptance and tracking
- [ ] Extend the existing focused Mastering browser workflow rather than creating one test per utility.
- [ ] Run Mastering/Music units, production type/build gate, and desktop/mobile Mastering browser acceptance.
