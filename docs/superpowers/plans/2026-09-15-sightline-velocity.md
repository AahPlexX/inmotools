...
- F6 acceptance delta 2026-09-16: file input now accepts multi-file batches, the entire Sightline workspace accepts dropped files/text, and the built-in sample control exposes a three-item sample library. The four focused desktop/mobile F6 browser cases passed locally after a fresh production build; branch CI will re-run the complete bounded lane on push.
